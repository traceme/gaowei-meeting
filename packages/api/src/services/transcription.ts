...
import { createWhisperEngine, WhisperService, LocalWhisperEngine, OpenAIWhisperEngine } from '@gaowei/whisper-engine'
...
export class LocalWhisperEngineAdapter implements TranscriptionEngine {
  ...
  async transcribe(audioBuffer: Buffer, filename: string, options?: TranscriptionOptions): Promise<TranscriptionResult> {
    try {
      ...
      throw new Error(`本地Whisper转录失败: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
...
export class OpenAIWhisperEngineAdapter implements TranscriptionEngine {
  ...
  async transcribe(audioBuffer: Buffer, filename: string, options?: TranscriptionOptions): Promise<TranscriptionResult> {
    try {
      ...
      throw new Error(`OpenAI Whisper转录失败: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
}
...
export class TranscriptionRouter {
  ...
  constructor(config: AppConfig) {
    ...
    if (config.ai.providers.openai?.apiKey) {
      this.engines.push(new OpenAIWhisperEngineAdapter(config.ai.providers.openai.apiKey))
    }
  }
  ...
  async transcribe(audioBuffer: Buffer, filename: string, options?: TranscriptionOptions): Promise<TranscriptionResult> {
    ...
    if (options?.engineType) {
      const targetEngine = this.engines.find(engine => {
        if (options.engineType === 'local' && engine.name === 'local-whisper') return true
        if (options.engineType === 'openai' && engine.name === 'openai-whisper') return true
        return false
      })
    ...
    for (const engine of this.engines) {
      try {
        if (await engine.isAvailable()) {
          ...
          const result = await engine.transcribe(audioBuffer, filename, options)
          console.log(`✅ ${engine.name} 转录成功，文本长度: ${result.text?.length || 0} 字符`)
          return result
        } else {
          console.log(`⚠️ ${engine.name} 引擎不可用，跳过`)
        }
      } catch (error) {
        lastError = error instanceof Error ? error : new Error(`${engine.name} 转录失败`)
    ...
    // 所有引擎都失败了
    throw new Error(`所有转录引擎都不可用: ${lastError?.message || '未知错误'}`)
  }

  async getEngineStatus(): Promise<Array<{ name: string; available: boolean; type: string }>> {
    const status = []
    for (const engine of this.engines) {
      try {
