...
def detect_device():
    ...
    if torch.cuda.is_available():
        device = "cuda"
        compute_type = "float16"  # GPU上使用float16更快
        logger.info(f"检测到CUDA设备: {torch.cuda.get_device_name(0)}")
    else:
        device = "cpu"
        compute_type = "int8"  # CPU上使用int8节省内存
        logger.info("使用CPU设备")
...
def create_app():
    ...
    # 从环境变量获取配置
    model_path = os.environ.get('MODEL_PATH', '/data2/hezm/project/gaowei-meeting-v2/gaowei-meeting-ai/packages/whisper-engine/src/python/faster-whisper-large-v3')
    ...
    # 初始化模型
    logger.info(f"初始化Whisper模型: {model_path}")
    ...
    def process_audio_with_progress(task_id: str, file_path: str, language: str = None, word_timestamps: bool = False):
        ...
        try:
            ...
            for i, segment in enumerate(segments_list):
                ...
                # GPU处理更快，减少延迟
                if total_segments > 10 and device == "cpu":  # 只对CPU长音频显示详细进度
                    time.sleep(0.05)
            ...
            logger.info(f"任务 {task_id} 转录完成 (设备: {device})")
    ...
    def transcribe():
        ...
        logger.info(f"转录参数 - 语言: {language} (whisper参数: {whisper_language}), 词级时间戳: {word_timestamps}, 设备: {device}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-path', type=str, 
                       default='/data2/hezm/project/gaowei-meeting-v2/gaowei-meeting-ai/packages/whisper-engine/src/python/faster-whisper-large-v3',
                       help='Path to the faster-whisper model.')
