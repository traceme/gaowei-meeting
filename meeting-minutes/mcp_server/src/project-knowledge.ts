...
export const KEY_FILES: ProjectFiles = {
  FRONTEND_MAIN_PAGE: {
    ...
    description: "The main page of the application, a large React component managing the meeting UI, state, and interaction with the Tauri backend.",
    language: 'typescript-react',
  },
  TAURI_BACKEND_LIB: {
    path: path.join(PROJECT_ROOT, 'frontend/src-tauri/src/lib.rs'),
    description: "The core of the Tauri backend in Rust. Manages audio capture, the transcription pipeline, and communication with the frontend and the C++ server.",
    language: 'rust',
  },
  WHISPER_SERVER_CPP: {
    path: path.join(PROJECT_ROOT, 'backend/whisper-custom/server/server.cpp'),
    description: "A high-performance C++ HTTP server that wraps the whisper.cpp library to provide a real-time transcription API.",
    language: 'cpp',
  },
  FRONTEND_PACKAGE_JSON: {
    path: path.join(PROJECT_ROOT, 'frontend/package.json'),
    description: "Defines the dependencies and scripts for the Next.js/Tauri frontend application.",
    language: 'json',
  }
};
